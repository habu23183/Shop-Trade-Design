...
$(document).ready(function() {
    for (var i = 0; i < products.length; i++) {
        /*get discount*/
        let a = Math.floor(((products[i]['compare_at_price'] - products[i]['price']) / products[i]['compare_at_price']) * 100);



        content = '<div class="box ' + products[i]['tag'] + '"><img src="' + products[i]['image_src'] + '" alt=""><div class="content"><h4>' + products[i]['vendor'] + '</h4><p class="text">' + products[i]['name'] + '</p><div class="cost"><p>$' + products[i]['price'] + '</p><small><span class="strike">$' + products[i]['compare_at_price'] + '</span> <span class="off">(' + a + '% Off)</span></small></div></div><div class="size"><button class="addCart  " type="button">Add To Cart</button><h4>Select Size</h4><div class="number"><span class="s">38</span><span class="s">39</span><span class="s">40</span class="s"><span class="s">44</span><span class="s">46</span></div><p class="text">Sizes: XS, S, M, L ,XL</p><div class="cost"><p>$' + products[i]['price'] + '</p><small><span class="strike">$' + products[i]['compare_at_price'] + '</span> <span class="off">(' + a + '% Off)</span></small></div></div>';




        document.getElementById('product-list').innerHTML += content;
        let num = document.getElementById('num').innerHTML = products.length;
    }
    ...
    for (let i = 0; i < addCount.length; i++) {
        ...
        addCount[i].addEventListener('click', (e) => {
            ...
            contentLi = '<li class="cart-li"><div class="close close-btn"><span></span><span></span></div><div class="img"><img src="' + products[i]['image_src'] + '" alt=""></div><div class="box"><p>' + products[i]['vendor'] + ' </p><p class="desc">' + products[i]['name'] + '</p><div class="cart-flex"><p class="cart-size">Size : <span></span></p><div class="count"><button id="minus">-</button><input id="count-input" type="text"><button id="plus">+</button></div></div></div><div class="cost"><h4>$ <span id="increase">' + products[i]['price'] + '</span></h4></div></li>';
            document.getElementById('cart-list-pdt').innerHTML += contentLi;
            ...
            plus.addEventListener('click', () => {
                countInput.value++;
                let num = countInput.value * products[i]['price'];
                let ins = document.getElementById('increase').innerText = num;
            })
